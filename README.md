...
| vcp-screener | Yes | **Fully recovered and verified end-to-end against live FMP data (2026-09-13).** Historical single-ticker scan on AAPL produced real detections with correct contraction math, forward-outcome P&L, and report generation. **Important corrected finding:** this free FMP key only has data access to a small curated ticker list (confirmed working: AAPL MSFT NVDA AMZN META GOOGL TSLA COST WMT), not "all stocks except ETFs" as first thought — ordinary S&P 500 names like MMM and APD fail on both quotes and historical data. A real "screen S&P 500" isn't possible on this plan regardless of code changes; see its SKILL.md for the full corrected writeup. |
