...
def _windows_chrome_paths() -> list[str]:
    """Known install locations for Chrome on Windows."""
    candidates = []
...
def open_browser(url: str) -> None:
    ...
    Windows: known Chrome install paths → default browser
    ...
    if sys.platform == "darwin":
        ...
        for chrome_path in _windows_chrome_paths():
            if os.path.isfile(chrome_path):
                try:
                    subprocess.Popen([chrome_path, url])  # nosec B603 — fixed local executable
                    return
                except OSError:
                    continue
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--subthemes",
        default=None,
        help="Comma-separated sub-theme slugs (e.g., aicloud,aicompute). Requires FinViz Elite "
        "(--elite or $FINVIZ_API_KEY) — FinViz silently ignores this filter on the Public tier.",
    )
    parser.add_argument(
        "--elite",
        action="store_true",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if subthemes and not elite:
        print(
            "Warning: --subthemes requires a FinViz Elite subscription. "
            "FinViz will silently ignore this filter on the Public (non-Elite) screener — "
            "pass --elite or set $FINVIZ_API_KEY if you have an Elite account.",
            file=sys.stderr,
        )
