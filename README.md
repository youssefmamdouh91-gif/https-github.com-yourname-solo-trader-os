...
| vcp-screener | **No** | SKILL.md only — `scripts/screen_vcp.py` (all detection/scoring logic) and 3 reference files are missing. Non-functional until recovered. This one was deliberately *not* reconstructed like trader-memory-core: it implements specific quantitative pattern-detection math (contraction depths, Stage-2 scoring thresholds) that the SKILL.md only describes at a high level — guessing at the actual numbers risks silently wrong screening results. |
