...
| vcp-screener | Yes | **Fully recovered and verified end-to-end against live FMP data (2026-09-13).** Historical single-ticker scan on AAPL produced real detections with correct contraction math, forward-outcome P&L, and report generation. Two free-tier FMP limits found along the way (not code bugs): ETF historical data (HYG) needs a paid plan, and the cross-sectional screener's batch-quote pre-filter also needs a paid plan — only `--history <TICKER>` mode is confirmed working on a free key. See its SKILL.md for details. |
