...
| vcp-screener | Yes | **Fully recovered** — all of `screen_vcp.py`, `scorer.py`, `report_generator.py`, `historical_scanner.py`, `historical_report.py`, `fmp_client.py`, `_fmp_compat.py`, the full `calculators/` package (8 files), and all 3 reference docs. `import screen_vcp` succeeds (confirmed by running it). Not yet run against a real FMP API key — `FMP_API_KEY` isn't set on this machine, so the queued HYG historical-scan smoke test (see its SKILL.md) hasn't actually executed yet. |
