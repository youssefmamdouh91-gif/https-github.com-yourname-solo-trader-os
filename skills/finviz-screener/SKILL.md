...
- Chrome-first browser opening with OS-appropriate fallbacks (macOS, Linux, Windows)
- Strict filter validation to prevent URL injection

**Known limitation:** Sub-theme filtering (`--subthemes`) is a FinViz Elite-only feature. On the Public tier FinViz silently drops this filter from the applied results rather than erroring — the script now prints a warning to stderr when `--subthemes` is used without `--elite`/`$FINVIZ_API_KEY`, but always confirm with the user whether they have an Elite account before relying on sub-theme drill-downs.
...
Note: this reference is a best-effort compilation (common codes + the mapping table below), not FinViz's official exhaustive list. If a code you need isn't there, fall back to the Common Concept Mapping table and general FinViz filter-code conventions (prefix + operator + value), and flag any uncertain code to the user before running the script.
...
| AI Cloud sub-theme (Elite only) | AIクラウド | `--subthemes "aicloud"` |
| AI Compute sub-theme (Elite only) | AI半導体 | `--subthemes "aicompute"` |
...
If any sub-theme filter is included and the user is not confirmed as Elite, note in this confirmation step that FinViz will likely ignore it on the Public tier.
...
# Theme + sub-theme + filters combined (sub-theme requires Elite)
python3 scripts/open_finviz_screener.py \
...
- `--subthemes` (optional): Comma-separated sub-theme slugs (e.g., `aicloud,aicompute`). Accepts bare slugs or `subtheme_`-prefixed values. **Requires FinViz Elite** — silently ignored by FinViz on the Public tier; the script prints a warning to stderr in that case.
...
3. Summary of applied filters (and whether any sub-theme filter was likely dropped due to Public tier)
...
**Requires FinViz Elite** — on the Public tier, `--subthemes` is silently dropped and only the theme + plain filters apply.
...
- `references/finviz_screener_filters.md` — Best-effort filter code reference (common codes + natural-language mapping); not FinViz's official exhaustive list — verify uncertain codes against the live screener UI before relying on them.
- `scripts/open_finviz_screener.py` — URL builder and Chrome opener (Windows/macOS/Linux)
