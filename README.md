...
| vcp-screener | Yes | **Fully recovered and verified end-to-end (2026-09-13).** A free-tier FMP key turned out to only have data access to a small curated ticker list (AAPL MSFT NVDA AMZN META GOOGL TSLA COST WMT) — not broader S&P 500/ETF coverage, and no code fix on our side could change that. Resolved by adding `scripts/yahoo_client.py`, a free/keyless alternative (Yahoo's public chart API) confirmed working for previously-blocked names (MMM, HYG, BRK-B) — now the **default** data source. `--data-source fmp` remains available for users with a paid FMP plan. See its SKILL.md for the full writeup. |
